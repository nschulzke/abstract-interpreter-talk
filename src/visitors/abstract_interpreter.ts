...
class AbstractInterpreterVisitor<TSources extends Record<string, AbstractTinyValue>> extends BaseCstVisitor {
  ...
  chooseExpression(ctx: ChooseExpressionCstChildren): AbstractTinyValue {
    ...
    const whenClauses =
      ctx.whenClause
      .map((clause, index) =>
        ({clause, predicateValue: predicates[index], consequentValue: consequents[index]}));

    // Check that all when clause predicates are of boolean type
    const nonBooleanWhenClauses =
      whenClauses
      .filter(({predicateValue}) =>
        !(predicateValue instanceof AbstractBoolean)
      );
    if (nonBooleanWhenClauses.length) {
      for (const {clause, predicateValue} of nonBooleanWhenClauses) {
        ...
        console.error(`When clause predicate \`${text}\` is of type ${typeName(predicateValue)}, not boolean at ${predicate.location?.startLine}:${predicate.location?.startColumn}`);
      }
      return undefined;
    }

    // Check that all when clause predicates are possible
    const impossibleWhenClauses =
      whenClauses
      .filter((whenClause, index) =>
        !(predicates[index] as AbstractBoolean).includes(true)
      );
    if (impossibleWhenClauses.length) {
      for (let {clause} of impossibleWhenClauses) {
        const predicate = clause.children.predicate[0];
        const text = this.input.slice(predicate.location!.startOffset!, predicate.location!.endOffset! + 1);
    ...
    const mismatchedWhenClauses = whenClauses.filter(({consequentValue}) =>
      typeName(consequentValue) !== typeName(whenClauses[0].consequentValue)
    );
    if (mismatchedWhenClauses.length) {
      for (const {clause, consequentValue} of mismatchedWhenClauses) {
        ...
        console.error(`When clause consequent \`${text}\` is of type ${typeName(consequentValue)} but expected ${typeName(whenClauses[0].consequentValue)} at ${consequent.location?.startLine}:${consequent.location?.startColumn}`);
      }
      return undefined;
    }
...
export function typeName(abstractValue: AbstractTinyValue): string {
  return abstractValue?.typeName ?? "undefined";
}
